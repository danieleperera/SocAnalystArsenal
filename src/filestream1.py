...
def ip_virustotal(ip: str, boolvalue: bool, domain: str = None, sha_sum: list = None) -> dict:
    """
    Documentation for ip_urlhaus.
    It gets one ip addresse at a time as a string,
    uses request to do a get request to ip_urlhaus,
...
def shodan_ip_info(ip: str) -> dict:
    data = get_api()
...
def apility_ip_info(ip: str) -> dict:
    data = get_api()
...
def hybrid_query(query, type: str = 'ip'):
